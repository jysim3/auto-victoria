...
export function PageHero({
  ...
  const localized = locale === 'zh' && zh ? zh : undefined
  const text = {
    eyebrow: localized?.eyebrow ?? eyebrow,
    title: localized?.title ?? title,
    description: localized?.description ?? description,
    primaryLabel: localized?.primaryLabel ?? primaryLabel,
    secondaryLabel: localized?.secondaryLabel ?? secondaryLabel,
  }
    ...
    <section className="relative overflow-hidden pt-36 pb-20">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
        <div className="max-w-4xl">
          <span className="mb-5 block text-sm uppercase tracking-[0.3em] text-primary">{text.eyebrow}</span>
          ...
          <p className="mt-7 max-w-3xl text-lg leading-relaxed text-muted-foreground">{text.description}</p>

          {(primaryHref && text.primaryLabel) || (secondaryHref && text.secondaryLabel) ? (
            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              {primaryHref && text.primaryLabel ? (
                renderLink(
                  primaryHref,
                  text.primaryLabel,
              ...
              {secondaryHref && text.secondaryLabel ? (
                renderLink(
                  secondaryHref,
                  text.secondaryLabel,
